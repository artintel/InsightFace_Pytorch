...
class MobileFaceNet(Module):
    ...
    def forward(self, x):
        out = self.conv1(
            x)  # shape: x.shape=[batch_size,channel,h,w]=[batch_size,3,112,112] out.shape = [batchsize,64,56,56]
        out = self.conv2_dw(out)  # [batch_size,64,56,56]
        ...
        out = self.conv_5(out)  # Residual:out.shape = [batch_size,128,7,7]

        out = self.conv_6_sep(out)  # out.shape = [batch_size,512,7,7]
